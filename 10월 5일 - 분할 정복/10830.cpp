...
matrix combine(int n, matrix m1, matrix m2) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            //(튜터용) 여기서 더하는 중간중간에 MOD연산을 안해도 되는 이유를 설명하려구요!
            for (int k = 0; k < n; k++) //최대 5*1000*1000 -> int
...
int main() {
    ...
    //출력
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) //(튜터용) 처음엔 MOD로 나누는거 안하고 반례 소개한 다음에 나누려구요!
            cout << ans[i][j] % MOD << ' '; //1000만 들어올 때
        cout << '\n';
    }
}