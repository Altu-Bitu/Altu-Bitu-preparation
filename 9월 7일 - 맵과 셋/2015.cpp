 ...
 * (실수 주의) k 는 우선 int 범위지만 연산 과정에서 범위 넘어가는 경우 있으므로 long long 써야하는 문제
 */

int main() {
    vector<ll> num, s; //num: 입력 배열, s: 부분합 배열
    map<ll, ll> m; //key: S[x] 즉 S[i] - k의 값, value: x의 개수
    ll n, k, ans = 0;
    ...
    for (int i = 1; i <= n; i++) {
        if (s[i] == k) //S[i] == k 일 경우
            ans++;
        if (m[s[i] - k] > 0) //S[x] = S[i] - k 를 만족하는 x가 존재한다면
            ans += m[s[i] - k]; //x의 개수만큼 ans 증가
