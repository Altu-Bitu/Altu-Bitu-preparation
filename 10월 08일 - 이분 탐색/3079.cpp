 ...
 *
 * left : 심사를 하는데 걸리는 시간인 Tk는 최소 1이다. 만약 모든 심사대의 심사 시간이 1이고, 친구들보다 입국심사대가 많다면 걸리는 시간은 1초
 * right : 심사가 가장 오래걸리는 심사대 k에서 모든 친구들이 한명씩 입국심사를 받는다면, 걸리는 시간은 k*m
 *
...
int main() {
    ...
    //연산 & 출력
    cout << lowerSearch(1, min_time * m, m);
}